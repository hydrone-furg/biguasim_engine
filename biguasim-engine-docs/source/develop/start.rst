 ...
 * Clone `biguasim_engine <git@github.com:hydrone-furg/biguasim_engine.git>`_.
...
You can now install the Python package by running ``pip install -e .`` (or ``pip install .``). 
Make sure not to skip this step - reinstalling the package is necessary to apply the changes from 
switching branches. 

Opening & Prepping Project 
==========================
To open the BiguaSim project in the Unreal Editor, find the ``Biguasim.uproject`` file in the 
``biguasim_engine`` directory. Double-click this file, and choose "5.3" if an engine version dialog
...
by right clicking on the ``Biguasim.uproject`` file within the engine folder, and then clicking on 
