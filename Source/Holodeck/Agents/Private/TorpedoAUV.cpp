...
/**
 * @brief Called every frame when the custom dynamics control scheme is active.
 * Converts linear and angular accelerations from the Python CommandArray into forces 
 * and torques, and applies them to the RootMesh.
...
/** * @brief Calculates and applies lift and drag forces for a specific stabilizing fin.
...
void ATorpedoAUV::ApplyFin(int i, float command){
    ...
    double sin = -UKismetMathLibrary::Sin(angle*3.14/180);
 ...
 * @param thrust The raw thrust command to apply.
 */
...
/**
 * @brief Enables linear and angular damping for the Torpedo's physical mesh.
 * Should be enabled when using the thruster and fins for proper fluid dynamics simulation.
 */
