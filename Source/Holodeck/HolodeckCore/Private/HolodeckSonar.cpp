...
void UHolodeckSonar::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		if (JsonParsed->HasTypedField<EJson::Boolean>("UseApprox")) {
			UseApprox = JsonParsed->GetBoolField("UseApprox");
		}
		...
		// --- NOVAS FLAGS GENÉRICAS ---
        if (JsonParsed->HasTypedField<EJson::Boolean>("SendGTIntensity")) {
            SendGTIntensity = JsonParsed->GetBoolField("SendGTIntensity");
        }
        if (JsonParsed->HasTypedField<EJson::Boolean>("SendGTElevation")) {
            SendGTElevation = JsonParsed->GetBoolField("SendGTElevation");
        }
        if (JsonParsed->HasTypedField<EJson::Boolean>("SendGTPointCloud")) {
            SendGTPointCloud = JsonParsed->GetBoolField("SendGTPointCloud");
        }
...
bool UHolodeckSonar::inRange(Octree* tree){
	...
	if(UseApprox){
		...
		tree->locSpherical.Z = UKismetMathLibrary::DegAtan2(locLocal.Size2D(), locLocal.Z);
	}
...
void UHolodeckSonar::shadowLeaves(){
    // Aqui a gente mantém a lógica de sorting e remoção de bins que você já tem
