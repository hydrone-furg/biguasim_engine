...
void UHolodeckSensor::Reset(){
	SecureZeroMemory(Buffer, GetNumItems() * GetItemSize());
}

void UHolodeckSensor::SecureZeroMemory(void* ptr, size_t size) {
    volatile char* p = static_cast<volatile char*>(ptr);
    while (size--) *p++ = 0;  // Ensures compiler does not optimize out
}