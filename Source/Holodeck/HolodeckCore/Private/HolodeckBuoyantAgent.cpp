...
void AHolodeckBuoyantAgent::InitializeAgent(){
	...
	if (!GetWorld()) {
    UE_LOG(LogHolodeck, Error, TEXT("BuoyantAgent: GetWorld() is NULL"));
    return;
	}

	if (!RootMesh) {
		UE_LOG(LogHolodeck, Error, TEXT("BuoyantAgent: RootMesh is NULL"));
		return;
	}

	if (!RootMesh->GetStaticMesh()) {
		UE_LOG(LogHolodeck, Error, TEXT("BuoyantAgent: StaticMesh is NULL"));
		return;
	}
