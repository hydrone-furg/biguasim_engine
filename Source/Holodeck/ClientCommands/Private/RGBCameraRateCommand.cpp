...
void URGBCameraRateCommand::Execute() {
	...
	verifyf(StringParams.size() == 3 && NumberParams.size() == 1, TEXT("URGBCameraRateCommand::Execute: Invalid Arguments"));
	...
	// std::string Detection = StringParams[2].c_str();
	// if(Detection != ""){
	// 	Camera->ClearMeshNames();
	// 	Camera->SetFilterMeshName(Detection);
	// 	Camera->GetDetections();
	// }
}
