    ...
    : max_distance_to_camera_(20000.f)
{
    ...
    SensorName = "FOVDetection";

}

void UDetectionComponent::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		if (JsonParsed->HasTypedField<EJson::String>("TargetDetection")) {
			target_detection_ = JsonParsed->GetStringField("TargetDetection");
		}
...
void UDetectionComponent::InitializeSensor()
{
    
    Super::InitializeSensor();
    Parent = this->GetAttachmentRootActor();
    this->Buffer = static_cast<char*>(UHolodeckSensor::Buffer);
    scene_capture_component_2D_ = CastChecked<USceneCaptureComponent2D>(SceneCapture);
    texture_target_ = SceneCapture->TextureTarget;
    object_filter_ = FObjectFilter();
    
    addMeshName(TCHAR_TO_UTF8(*target_detection_));
}

void UDetectionComponent::TickSensorComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
        ...
        if (object_filter_.matchesActor(actor)) {
            if (FVector::Distance(actor->GetActorLocation(), GetComponentLocation()) <= max_distance_to_camera_) {
                ...
                if (texture_target_ && calcBoundingFromViewInfo(actor, box_2D_out)) {
                    ...
                    FVector2D box2D_min = box_2D_out.Min;
                    FVector2D box2D_max = box_2D_out.Max;

                    FBox box_3D_ = actor->GetComponentsBoundingBox(true);
                    FBox box_3D = FBox(getRelativeLocation(box_3D_.Min), getRelativeLocation(box_3D_.Max));

                    FVector box3D_min = box_3D.Min;
                    FVector box3D_max = box_3D.Max;
                    ...
                    element += R"({"box2D": {"min": [)" + std::to_string(box2D_min.X) + R"(,)" + std::to_string(box2D_min.Y) + R"(], "max": [)" + std::to_string(box2D_max.X) + R"(,)" + std::to_string(box2D_max.Y) + R"(]}, "box3D": {"min": [)" + std::to_string(box3D_min.X) + R"(,)" + std::to_string(box3D_min.Y) + R"(,)" + std::to_string(box3D_min.Z) + R"(], "max": [)" + std::to_string(box3D_max.X) + R"(,)" + std::to_string(box3D_max.Y) + R"(,)" + std::to_string(box3D_max.Z) + R"(]}, "tag": ")" + TCHAR_TO_UTF8(*actor->Tags[0].ToString()) + R"("})";


                    std::string name = TCHAR_TO_UTF8(*actor->GetName());
                    msg += R"(")" + name + R"(": )" + element;
                    isFirst = false;
    ...
    // StringBuffer[detections] = "}";

    std::strcpy(StringBuffer, msg.c_str());
...
//     getDetections(StringBuffer);
// }

void UDetectionComponent::getDetections(std::string* buffer)
// const TArray<FDetectionInfo>& UDetectionComponent::getDetections(std::string* buffer)
{
        ...
        if (object_filter_.matchesActor(actor)) {
            if (FVector::Distance(actor->GetActorLocation(), GetComponentLocation()) <= max_distance_to_camera_) {
                ...
                if (texture_target_ && calcBoundingFromViewInfo(actor, box_2D_out)) {
                    ...
                    buffer[dtids] = TCHAR_TO_UTF8(*actor->GetName());
                    dtids++;
                }
            }
        }
    ...
    dtids = 0;
    if(!cached_detections_.Num()){
        buffer[dtids] = "None";
    }
    // FMemory::Memcpy(Buffer, &cached_detections_[0], cached_detections_.Num() * sizeof(std::string));
...
bool UDetectionComponent::calcBoundingFromViewInfo(AActor* actor, FBox2D& box_out)
{
    ...
    // Build bounding box from visible primitive components
    ...
    {
        if (comp->IsRegistered() && comp->IsVisible())
        {
    ...
    FVector origin = combinedBox.GetCenter();
    FVector extend = combinedBox.GetExtent();
    ...
    UE_LOG(LogHolodeck, Log, TEXT("Is in camera View: %s"),
    is_in_camera_view ? TEXT("true") : TEXT("false"));
